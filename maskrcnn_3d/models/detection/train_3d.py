...
from maskrcnn_3d.utils.eval import accuracy, get_preds, mpjpe, get_preds_3d
import cv2
...
def step(split, epoch, opt, data_loader, model, optimizer=None):
    ...
    #### {'input': inp, 'target': out, 'meta': meta, 'reg_target': reg_target, 'reg_ind': reg_ind, 'reg_mask': reg_mask}
    for i, batch in enumerate(data_loader):
        ...
        #### gt_2d = batch['meta']['pts_crop'].cuda(device=opt.device, non_blocking=True).float() / opt.output_h
        gt_2d = batch['meta']['pts_crop'].cuda().float() / opt.output_h
        output = model(batch['input'])

        loss = crit(output[-1]['hm'], batch['target'])  ### 计算2d损失（两个【16,64,64】之间的损失）
        # print('1: ',output[-1]['depth'])
        # print('2: ',batch['reg_mask'])
        # print('3: ',batch['reg_ind'])
        # print('4: ',batch['reg_target'])
        loss_3d = crit_3d(output[-1]['depth'].cpu(), batch['reg_mask'].cpu(), batch['reg_ind'].cpu(), batch['reg_target'].cpu(), gt_2d.cpu())
        ...
        if split == 'train':
            ...
            #### output_depth_flip = torch.from_numpy(output_depth_flip).cuda(device=opt.device, non_blocking=True)
