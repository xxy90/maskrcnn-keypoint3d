...
class KeypointRCNN(FasterRCNN):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if depth_predictor is None:
            ...
            depth_predictor = DepthMapPredictor(keypoint_dim_reduced, num_keypoints)
        ...
        # self.roi_heads.depth_roi_pool=depth_roi_pool
        # self.roi_heads.depth_head=depth_head
        self.roi_heads.depth_predictor=depth_predictor
...
class KeypointRCNNPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
class DepthMapHeads(nn.Sequential):
    def __init__(self, in_channels, layers):
        d = []
        next_feature = in_channels
        for l in layers:
            ...
            d.append(nn.ReLU(inplace=True))
            next_feature = l
        super(DepthMapHeads, self).__init__(*d)
        for m in self.children():
            if isinstance(m, misc_nn_ops.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode="fan_out", nonlinearity="relu")
                nn.init.constant_(m.bias, 0)
...
class DepthMapPredictor(nn.Module):
    def __init__(self, in_channels, num_keypoints):
        super(DepthMapPredictor, self).__init__()
        input_features = in_channels
        deconv_kernel = 4
        self.kps_score_lowres = misc_nn_ops.ConvTranspose2d(
            input_features,
...
def keypointrcnn_resnet50_fpn_3d(pretrained=False, progress=True,
                              num_classes=2, num_keypoints=16,
                              pretrained_backbone=True,finetune=False, **kwargs):
    """
    ...
    if finetune:
        print('model is finetuning')
        pre_rcnn_dict= load_state_dict_from_url(model_urls['keypointrcnn_resnet50_fpn_coco'],progress=progress)
        model_dict=model.state_dict()

        #roi_heads.keypoint_predictor.kps_score_lowres.bias/weight
        #roi_heads.keypoint_head

        #pre_rcnn_dict={k:v for k,v in pre_rcnn_dict.items() if (k in model_dict and 'keypoint' not in k )}
        pre_rcnn_dict = {k: v for k, v in pre_rcnn_dict.items() if (k in model_dict and 'backbone' in k)}

        for name, p in model.named_parameters():

            if pre_rcnn_dict.__contains__(name):
                p.requires_grad = False
                print('the no grad param is ', name)
            else:
                p.requires_grad = True

        model_dict.update(pre_rcnn_dict)
    ...
    if pretrained:
        #state_dict = load_state_dict_from_url(model_urls['keypointrcnn_resnet50_fpn_coco'], progress=progress)
        state_dict=torch.load('/data/ai/xxy/mask_rcnn/maskrcnn_3d/exp/default/model_last.pth')
