    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if keypoint_roi_pool is None:
            keypoint_roi_pool = MultiScaleRoIAlign(
                ...
                output_size=16,
                sampling_ratio=2)
        if depth_roi_pool is None:
            depth_roi_pool=MultiScaleRoIAlign(
                featmap_names=[0, 1, 2, 3],
                output_size=16,
                sampling_ratio=2)
        ...
        if depth_head is None:
            keypoint_layers = tuple(512 for _ in range(8))
            depth_head = DepthMapHeads(out_channels, keypoint_layers)
...
def keypointrcnn_resnet50_fpn_3d(pretrained=False, progress=True,
    ...
    if finetune:
        ...
        pre_rcnn_dict={k:v for k,v in pre_rcnn_dict.items() if (k in model_dict and not k.find('predictor'))}
    ...
    if pretrained:
        ...
        state_dict=torch.load('/data/ai/xxy/mask_rcnn/maskrcnn_3d/exp/default/model_last_0.pth')
