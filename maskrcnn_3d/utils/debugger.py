...
def show_2d(img, points, c, edges):
    ...
    for j in range(num_joints):
        ...
        cv2.putText(img, str(j), (points[j, 0], points[j, 1]), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)
...
def show_2d_box(img, box):
    cv2.rectangle(img, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (180, 255, 45), 2)
    return img
def show_2d_new(img, points, c, pred_conf, edges):
    num_joints = points.shape[0]
    points = ((points.reshape(num_joints, -1))).astype(np.int32)
    for j in range(num_joints):
      if pred_conf[j]>0.5:
...
class Debugger(object):
    ...
    def add_box_2d(self, box, imgId='default'):
        self.imgs[imgId] = show_2d_box(self.imgs[imgId], box)

    def add_point_2d_new(self, point, c, pred_conf, imgId='default'):
        self.imgs[imgId] = show_2d_new(self.imgs[imgId], point, c, pred_conf,self.edges)
    ...
    def save_img(self, imgId='default', path='../debug/'):
        cv2.imwrite(path + '/{}.jpg'.format(imgId), self.imgs[imgId])

    def save_all_imgs(self, path='../debug/'):
        for i, v in self.imgs.items():
            cv2.imwrite(path + '/{}.jpg'.format(i), v)
