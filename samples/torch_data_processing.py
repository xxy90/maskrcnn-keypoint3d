...
class PostprocessYOLO(object):
    ...
    def _reshape_output(self, output):
        ...
        and then return it in (3,height,width,85) dimensionality after further reshaping.

        Keyword argument:
        output -- an output from a TensorRT engine after inference
        """

        output=self.FloatTensor(output)
        ...
        return output.contiguous().view([dim1, dim2,dim3, dim4])
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        image_dims = self.FloatTensor([width, height, width, height])
        ...
        idxs = torch.eq(categories , 0)## peoples label is 0
        ...
        categories = torch.cat(nms_categories)
        confidences = torch.cat(nscores)
        # boxes=boxes.numpy()
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        anchors = self.FloatTensor([self.anchors[i] for i in mask])

        # Reshape to N, height, width, num_anchors, box_params:
        anchors_tensor = anchors.view( [1, 1,len(anchors),  2])
        ...
        box_confidence = box_confidence[...,None]
        box_class_probs = sigmoid(output_reshaped[..., 5:])

        col = torch.arange(grid_w).repeat(grid_w, 1).view([ grid_w, grid_w,1,1]).type(self.FloatTensor)
        row = torch.arange(grid_h).repeat(grid_h, 1).t().view([ grid_h, grid_h,1,1]).type(self.FloatTensor)

        col = col.repeat(1,1,3,1)
        row = row.repeat(1,1,3,1)
        grid = torch.cat((col, row), dim=-1)

        box_xy += grid
        #box_xy /= self.FloatTensor(grid_w, grid_h)
        box_xy[..., 0] = box_xy[..., 0] / grid_w
        box_xy[..., 1] = box_xy[..., 1] / grid_h
        #box_wh /= self.FloatTensor(self.input_resolution_yolo)
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        box_class_scores,indices = torch.max(box_scores, dim=-1)

        pos = torch.ge(box_class_scores , self.object_threshold)
    ...
    def _nms_boxes(self, boxes, box_confidences):
        ...
        sorted,ordered=torch.sort(box_confidences,descending=True)
        print(box_confidences.shape,ordered)
        keep = list()
        while ordered.nelement()!=0 :
            # Index of the current element:

            i = ordered[0]
            keep.append(i)
            xx1 = torch.max(x_coord[i], x_coord[ordered[1:]])
            yy1 = torch.max(y_coord[i], y_coord[ordered[1:]])
            xx2 = torch.min(x_coord[i] + width[i], x_coord[ordered[1:]] + width[ordered[1:]])
            yy2 = torch.min(y_coord[i] + height[i], y_coord[ordered[1:]] + height[ordered[1:]])
            #print('min x ,y is ',xx2 ,yy2 )
            width1 = torch.max(torch.zeros(1).cuda(), xx2 - xx1 + 1)
            height1 = torch.max(torch.zeros(1).cuda(), yy2 - yy1 + 1)
            ...
            print(iou.shape,iou)
            indexes = torch.le(iou,self.nms_threshold)

            id=torch.arange(0,indexes.nelement())[indexes]
            if id.nelement()!=0:
                ordered = ordered[id + 1]
            else:
                break
