...
class PreprocessYOLO(object):
    ...
    def _shuffle_and_normalize(self, image):
        ...
        image = np.array(image, dtype=cp.float32, order='C')
        return image
...
class PostprocessYOLO(object):
    ...
    def _reshape_output(self, output):
        ...
        ## the output shape is [1,255,19,19],and output is torch tensor should be [3,19,19,85]
        Keyword argument:
        output -- an output from a TensorRT engine after inference
        """
        output = cp.transpose(output, [0, 2, 3, 1])
        ...
        return cp.reshape(output,(dim1, dim2, dim3, dim4))
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        boxes = cp.concatenate(boxes)
        categories = cp.concatenate(categories)
        confidences = cp.concatenate(confidences)

        # Scale boxes back to original image shape:
        width, height = resolution_raw
        image_dims = cp.array([width, height, width, height])
        ...
        idxs = cp.where(categories == 0)## peoples label is 0
        ...
        boxes = cp.concatenate(nms_boxes)
        categories = cp.concatenate(nms_categories)
        confidences = cp.concatenate(nscores)
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        box_classes = cp.argmax(box_scores, axis=-1)
        box_class_scores = cp.max(box_scores, axis=-1)
        pos = cp.where(box_class_scores >= self.object_threshold)
    ...
    def _nms_boxes(self, boxes, box_confidences):
        ...
        while ordered.size > 0:
            ...
            xx2 = cp.minimum(x_coord[i] + width[i], x_coord[ordered[1:]] + width[ordered[1:]])
            yy2 = cp.minimum(y_coord[i] + height[i], y_coord[ordered[1:]] + height[ordered[1:]])
