...
def huffman_encode(arr, prefix, save_dir='./'):
    ...
    datasize = dump(data_encoding, directory/'{prefix}.bin')

    # Dump codebook (huffman tree)
    codebook_encoding = encode_huffman_tree(root, dtype)
    treesize = dump(codebook_encoding, directory/'{prefix}_codebook.bin')
...
def huffman_decode(directory, prefix, dtype):
    ...
    # Read the codebook
    codebook_encoding = load(directory/'{prefix}_codebook.bin')
    root = decode_huffman_tree(codebook_encoding, dtype)

    # Read the data
    data_encoding = load(directory/'{prefix}.bin')
...
def float2bitstr(f):
    ...
    return ''.join('{byte:08b}' for byte in four_bytes) # string of '0's and '1's
...
def huffman_encode_model(model, directory='encodings/'):
    ...
    original_total = 0
    compressed_total = 0
    print("{'Layer':<15} | {'original':>10} {'compressed':>10} {'improvement':>11} {'percent':>7}")
    print('-'*70)
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Encode
            t0, d0 = huffman_encode(mat.data, name+'_{form}_data', directory)
            t1, d1 = huffman_encode(mat.indices, name+'_{form}_indices', directory)
            t2, d2 = huffman_encode(calc_index_diff(mat.indptr), name+'_{form}_indptr', directory)

            # Print statistics
            original = mat.data.nbytes + mat.indices.nbytes + mat.indptr.nbytes
            compressed = t0 + t1 + t2 + d0 + d1 + d2

            print("{name:<15} | {original:10} {compressed:10} {original / compressed:>10.2f}x {100 * compressed / original:>6.2f}%")
        else: # bias
            # Note that we do not huffman encode bias
            bias = param.data.cpu().numpy()
            bias.dump('{directory}/{name}')
    ...
    print('-'*70)
    print("{'total':15} | {original_total:>10} {compressed_total:>10} {original_total / compressed_total:>10.2f}x {100 * compressed_total / original_total:>6.2f}%")
...
def huffman_decode_model(model, directory='encodings/'):
    for name, param in model.named_parameters():
        ...
        if 'weight' in name:
            ...
            # Decode data
            data = huffman_decode(directory, name+'_{form}_data', dtype='float32')
            indices = huffman_decode(directory, name+'_{form}_indices', dtype='int32')
            indptr = reconstruct_indptr(huffman_decode(directory, name+'_{form}_indptr', dtype='int32'))
