...
class opts():
  def __init__(self):
    ...
    self.parser.add_argument('--gpus', default='6', help='-1 for CPU')
    ...
    self.parser.add_argument( '--weight-decay', default=1e-4, type=float)
    self.parser.add_argument('--momentum', default=0.9, type=float, metavar='M', help='momentum')
    # self.parser.add_argument('--arch', default = 'msra_50', help = 'hg | msra_xxx')
    self.parser.add_argument('--arch', default = 'msra_101', help = 'hg | msra_xxx')

    self.parser.add_argument('--disable_cudnn', action = 'store_true')
    self.parser.add_argument('--save_all_models', action = 'store_true')
    self.parser.add_argument('--print_iter', type = int, default = -1, help = 'for run in cloud server')
  ...
  def parse(self, args = ''):
    ...
    opt.exp_dir = '/data/ai/xxy/mask_rcnn/maskrcnn_3d/exp'
    ...
    if opt.input_h == -1 and opt.input_w == -1 and \
      ...
      if opt.dataset == 'coco':
        ...
        opt.input_h, opt.input_w = 224, 224
